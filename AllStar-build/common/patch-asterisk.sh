...
# Increase frame size in usbradio driver
sed -i 's/define FRAME_SIZE	160/define FRAME_SIZE	200/' ./channels/chan_usbradio.c
# Increase queue size in usbradio driver
sed -i 's/define	QUEUE_SIZE	2/define	QUEUE_SIZE	6/' ./channels/chan_usbradio.c
echo "Done"
exit 0
